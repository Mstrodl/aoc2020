fn main() {
  let input: String = String::from("6-7 w: wwhmzwtwwk
...
1-18 c: ccdczcvccvclcccvkccb");
  ...
  for line in input.split("\n") {
    ...
    let minNum: usize = usize::from_str_radix(&min, 10).unwrap();
    let maxNum: usize = usize::from_str_radix(&max, 10).unwrap();

    let mut valid: i8 = 0;
    if chars[minNum-1+passwordIndex] == letter {
      valid += 1;
    }
    if chars[maxNum-1+passwordIndex] == letter {
      valid += 1;
    }
    if valid == 1 {
      validCount += 1;
